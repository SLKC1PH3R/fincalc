...
function ComparePanel({ sims, onClose }: { sims: [Simulation, Simulation]; onClose: () => void }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden mb-2" style={{ border: '1px solid rgba(176,120,32,0.20)', background: 'rgba(176,120,32,0.05)' }}>
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(176,120,32,0.13)' }}>
        <div className="flex items-center gap-2">
          <GitCompare className="h-4 w-4" style={{ color: '#B07820' }} />
          <span style={{ fontSize: 13, fontWeight: 600, color: '#B07820' }}>Comparaison — {config.label}</span>
        </div>
...
function SimCard({ sim, onDelete, selected, onSelect, selectMode }: { sim: Simulation; onDelete: () => void; selected: boolean; onSelect: () => void; selectMode: boolean }) {
  ...
  return (
    <Card className="group" style={selected ? { borderColor: 'rgba(176,120,32,0.40)', background: 'rgba(176,120,32,0.05)' } : {}}>
      <CardContent className="pt-5 pb-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            {selectMode && (
              <button onClick={onSelect} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, flexShrink: 0, marginTop: 2, color: selected ? '#B07820' : 'rgba(255,255,255,0.25)' }}>
...
export default function HistoryPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in px-7 py-8" style={{ background: 'var(--p-bg)' }}>
