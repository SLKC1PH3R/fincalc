...
function ScenarioRow({ s, selected, onClick, showAllCols }: { s: RetirementScenario; selected: boolean; onClick: () => void; showAllCols: boolean }) {
  ...
  return (
    <tr
      onClick={onClick}
      style={{ cursor: 'pointer', background: selected ? 'rgba(255,255,255,0.04)' : 'transparent', borderBottom: '1px solid var(--p-line)' }}
    >
      <td style={{ padding: '10px 12px', fontSize: 13, fontWeight: 500, fontVariantNumeric: 'tabular-nums', color: 'var(--p-text-em)' }}>{s.age} ans</td>
      {showAllCols && <td style={{ padding: '10px 12px', fontSize: 13, color: 'var(--p-text-dim)', fontVariantNumeric: 'tabular-nums' }}>{s.trimestres}</td>}
      ...
      {showAllCols && <td style={{ padding: '10px 12px', fontSize: 13, fontVariantNumeric: 'tabular-nums', textAlign: 'right', color: 'var(--p-text-em)' }}>{fmtEur(s.pensionBase)}</td>}
      {showAllCols && <td style={{ padding: '10px 12px', fontSize: 13, fontVariantNumeric: 'tabular-nums', textAlign: 'right', color: 'var(--p-text-em)' }}>{fmtEur(s.pensionArrco)}</td>}
      <td style={{ padding: '10px 12px', fontSize: 13, fontVariantNumeric: 'tabular-nums', textAlign: 'right', fontWeight: 600, color: 'var(--p-text)' }}>{fmtEur(s.pensionBrute)}</td>
      {showAllCols && <td style={{ padding: '10px 12px', fontSize: 13, fontVariantNumeric: 'tabular-nums', textAlign: 'right', color: 'var(--p-text-dim)' }}>{s.replacementRate.toFixed(0)}%</td>}
    </tr>
  )
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '310px 1fr 290px', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ position: 'sticky', top: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label style={{ fontSize: 11, color: 'var(--p-text-dim)' }}>Âge actuel</label>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--p-text-em)' }}>{inputs.salaryGrowth.toFixed(1)}%/an</span>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <label style={{ fontSize: 11, color: 'var(--p-text-dim)' }}>Âge de départ simulé</label>
                    <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--p-text-em)' }}>{inputs.departureAge} ans</span>
                  </div>
            ...
            <div style={{ background: `${COLOR}0d`, border: `1px solid ${COLOR}25`, borderRadius: 12, padding: '12px 14px' }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--p-text-em)', marginBottom: 10 }}>Progression vers le taux plein</p>
              {[
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
              {[
                ...
                { label: 'Base CNAV', value: `${fmtEur(main.pensionBase)}/mois`, color: 'var(--p-text)' },
                { label: 'Agirc-Arrco', value: `${fmtEur(main.pensionArrco)}/mois`, color: 'var(--p-text)' },
              ].map((kpi, i) => (
            ...
            <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--p-line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--p-text-em)', margin: 0 }}>Scénarios de départ — 62 à 70 ans</p>
                  <p style={{ fontSize: 11, color: 'var(--p-text-dim)', marginTop: 2 }}>Cliquez sur un âge pour le sélectionner</p>
                </div>
                <button onClick={() => setShowAllCols(v => !v)} style={{ fontSize: 11, color: 'var(--p-text-dim)', background: 'none', border: '1px solid var(--p-line)', borderRadius: 6, padding: '3px 9px', cursor: 'pointer' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: 'rgba(255,255,255,0.03)' }}>
                      ...
                      {showAllCols && <th style={{ padding: '8px 12px', textAlign: 'right', fontSize: 11, fontWeight: 500, color: 'var(--p-text-dim)', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid var(--p-line)' }}>Base CNAV</th>}
                      {showAllCols && <th style={{ padding: '8px 12px', textAlign: 'right', fontSize: 11, fontWeight: 500, color: 'var(--p-text-dim)', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid var(--p-line)' }}>Agirc-Arrco</th>}
                      <th style={{ padding: '8px 12px', textAlign: 'right', fontSize: 11, fontWeight: 500, color: 'var(--p-text-dim)', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid var(--p-line)' }}>Total brut</th>
                      {showAllCols && <th style={{ padding: '8px 12px', textAlign: 'right', fontSize: 11, fontWeight: 500, color: 'var(--p-text-dim)', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid var(--p-line)' }}>Remplac.</th>}
                    </tr>
                  </thead>
                  <tbody>
                    {r.scenarios.map(s => (
            ...
            <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--p-text-em)' }}>Détail — départ à {main.age} ans</span>
                <span style={{ fontSize: 12, color: 'var(--p-text-dim)' }}>{main.trimestres} trimestres validés</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 24px' }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--p-text-em)', fontVariantNumeric: 'tabular-nums' }}>{row.value}</span>
                  </div>
                ))}
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: 'var(--p-card)', border: `1px solid ${scoreConf.borderColor}`, borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--p-text-em)', margin: 0 }}>
            ...
            <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 12, padding: '14px 16px' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--p-text-em)', marginBottom: 10 }}>Sources de revenus retraite</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
                {donutData.map((d, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--p-text-em)' }}>{fmtEur(d.value)}/mois</span>
                  </div>
                ))}
