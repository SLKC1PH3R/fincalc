    ...
    <div style={{ padding: '20px 24px 48px' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--p-text)', margin: 0, letterSpacing: '-0.3px' }}>Crédit à la Consommation</h1>
              <p style={{ fontSize: 12, color: 'var(--p-text-dim)', margin: 0 }}>Coût total · TAEG · Capacité de remboursement</p>
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '270px 1fr 290px', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <Label style={{ fontSize: 11, color: 'var(--p-text-dim)' }}>Montant emprunté (€)</Label>
              ...
              {/* TAEG */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <Label style={{ fontSize: 11, color: 'var(--p-text-dim)' }}>TAEG annuel (%)</Label>
                <Input type="number" value={taeg} onChange={e => setTaeg(Number(e.target.value))} min={0} max={50} step={0.1} style={{ height: 36, fontSize: 13 }} />
                <p style={{ fontSize: 11, color: 'var(--p-text-dim)', margin: 0 }}>Taux Annuel Effectif Global — mentionné dans votre offre de crédit</p>
                  ...
                  {([{ label: 'Perso', val: 5.5 }, { label: 'Auto', val: 8.5 }, { label: 'Renouv.', val: 18 }] as const).map(s => (
                    <button key={s.label} onClick={() => setTaeg(s.val)}
                      style={{ flex: 1, padding: '3px 0', borderRadius: 6, fontSize: 10, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s',
                        ...
                        border: taeg === s.val ? `1px solid ${COLOR}35` : '1px solid var(--p-line)',
                        color: taeg === s.val ? COLOR : 'var(--p-text-dim)' }}>
              ...
              {/* Durée */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <Label style={{ fontSize: 11, color: 'var(--p-text-dim)' }}>Durée (mois)</Label>
                ...
                <p style={{ fontSize: 11, color: 'var(--p-text-dim)', margin: 0 }}>{Math.floor(durationMonths / 12)} an{Math.floor(durationMonths / 12) > 1 ? 's' : ''} {durationMonths % 12 > 0 ? `et ${durationMonths % 12} mois` : ''}</p>
                <div style={{ display: 'flex', gap: 4 }}>
                  {([12, 24, 48, 60] as const).map(m => (
                    <button key={m} onClick={() => setDurationMonths(m)}
                      style={{ flex: 1, padding: '3px 0', borderRadius: 6, fontSize: 10, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s',
                        ...
                        border: durationMonths === m ? `1px solid ${COLOR}35` : '1px solid var(--p-line)',
                        color: durationMonths === m ? COLOR : 'var(--p-text-dim)' }}>
                      {m}m
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <Label style={{ fontSize: 11, color: 'var(--p-text-dim)' }}>Rendement placement alternatif (%/an)</Label>
                ...
                <p style={{ fontSize: 11, color: 'var(--p-text-dim)', margin: 0 }}>Ce que le capital aurait rapporté (ETF world, PEA…)</p>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: `${COLOR}0d`, border: `1px solid ${COLOR}25`, borderRadius: 12, padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                ...
                { label: 'Total remboursé', value: fmt(res.totalPaid), color: 'var(--p-text-em)' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {[
              ...
              { label: 'Total remboursé', value: fmt(res.totalPaid), sub: `dont ${interestRatio.toFixed(1)}% d'intérêts`, color: 'var(--p-text)' },
              { label: 'Intérêts payés', value: fmt(res.totalInterest), sub: `${interestRatio.toFixed(1)}% du capital`, color: 'var(--p-text)' },
              { label: 'TAEG', value: `${taeg}%`, sub: 'Taux effectif global', color: 'var(--p-text)' },
            ].map((kpi, i) => (
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 14, padding: '14px 16px' }}>
            <div style={{ paddingBottom: 12, borderBottom: '1px solid var(--p-line)', marginBottom: 14, display: 'flex', alignItems: 'baseline', gap: 8 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--p-text-em)', margin: 0 }}>Évolution du remboursement</p>
              <p style={{ fontSize: 11, color: 'var(--p-text-dim)', margin: 0 }}>Capital restant dû vs total remboursé</p>
            </div>
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 14, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
              <TrendingDown style={{ width: 14, height: 14, color: 'var(--p-text-dim)' }} />
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--p-text-em)', margin: 0 }}>Décomposition du coût réel</p>
            </div>
            {[
              ...
              <div key={i} style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                  ...
                  <span style={{ color: 'var(--p-text-em)', fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{fmt(item.value)}</span>
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 14, padding: '14px 16px' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--p-text-em)', margin: '0 0 12px' }}>Répartition du coût réel</p>
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--p-line)' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--p-text-em)', margin: 0 }}>Conseils pour réduire le coût</p>
            </div>
