  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--p-text-dim)', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600, marginBottom: 2 }}>
      ...
      {tab === 'patrimoine' && patrimoineNet != null && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ height: 8, background: 'var(--p-line)', borderRadius: 99, overflow: 'hidden', marginBottom: 6 }}>
              <div style={{ width: `${pct}%`, height: '100%', background: `linear-gradient(90deg, #f87171, #fb923c, #B07820, #34d399)`, borderRadius: 99, transition: 'width 0.5s ease' }} />
            </div>
            {/* Your marker */}
            <div style={{ position: 'absolute', top: -2, left: `calc(${pct}% - 6px)`, width: 12, height: 12, borderRadius: '50%', background: pctColor, border: '2px solid var(--p-card)', boxShadow: `0 0 8px ${pctColor}60` }} />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
            {[
              ...
              { label: '3ème quartile', value: 440_000, color: '#B07820' },
              ...
              <div key={bench.label} style={{ padding: '8px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 8, textAlign: 'center' as const }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--p-text-faint)' }}>{bench.label}</div>
              </div>
            ))}
          </div>
      ...
      {(tab === 'epargne' || patrimoineNet == null) && tauxEpargne != null && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
            <div>
              <div style={{ fontSize: 11, color: 'var(--p-text-dim)', marginBottom: 2 }}>Votre taux</div>
            ...
            <div>
              <div style={{ fontSize: 11, color: 'var(--p-text-dim)', marginBottom: 2 }}>Moyenne française</div>
              <div style={{ fontSize: 28, fontWeight: 800, color: '#B07820', fontVariantNumeric: 'tabular-nums' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[
              ...
              { label: 'Moyenne FR', value: FRENCH_AVERAGES.tauxEpargne, color: '#B07820' },
            ].map(row => (
              <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 11, color: 'var(--p-text-dim)', width: 80, flexShrink: 0 }}>{row.label}</span>
                <div style={{ flex: 1, height: 8, background: 'var(--p-line)', borderRadius: 99, overflow: 'hidden' }}>
