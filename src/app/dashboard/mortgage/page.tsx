    ...
    <div style={{ padding: '20px 24px 48px' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.3px' }}>Prêt Immobilier</h1>
              <p style={{ fontSize: 12, color: 'var(--text-muted-c)', margin: 0 }}>Mensualités · TAEG · Coût total du crédit</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', flexShrink: 0 }}>
            ...
            <Button variant="outline" size="sm" style={{ borderColor: 'var(--card-dark-border)', color: 'var(--text-muted-c)' }} onClick={() => setInputs({ amount: 240000, rate: 3.5, years: 20, insurance: 80, fees: 5000 })}>
              Réinitialiser
            </Button>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '270px 1fr 290px', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <p style={{ fontSize: 10, color: 'var(--text-subtle)', margin: 0 }}>Taux moyens France — mars 2026</p>
              </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted-c)' }}>
                  {[15, 20, 25].map(y => (
                    <button key={y} onClick={() => set('years')(y)}
                      style={{ background: inputs.years === y ? `${COLOR}18` : 'transparent', border: inputs.years === y ? `1px solid ${COLOR}35` : '1px solid transparent', borderRadius: 5, padding: '2px 8px', cursor: 'pointer', color: inputs.years === y ? COLOR : 'var(--text-muted-c)', fontSize: 11, transition: 'all 0.15s' }}>
                      {y} ans
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <p style={{ fontSize: 10, color: 'var(--text-subtle)', margin: 0 }}>0.10–0.40% du capital / an · délégation conseillée</p>
              </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {[
              ...
              { label: 'Coût total crédit', value: fmt(r.totalCost), sub: null, color: 'var(--text-primary)' },
              { label: 'Total intérêts', value: fmt(r.totalInterest), sub: `${fmtPct(interestRatio)} du capital`, color: 'var(--text-primary)' },
              { label: 'TAEG', value: `${r.taeg.toFixed(2)}%`, sub: 'Taux effectif global', color: 'var(--text-primary)' },
            ].map((kpi, i) => (
              <div key={i} style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 12, padding: '12px 14px' }}>
                ...
                {kpi.sub && <p style={{ fontSize: 10, color: 'var(--text-muted-c)', margin: 0 }}>{kpi.sub}</p>}
              </div>
            ))}
          </div>
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, padding: '14px 16px' }}>
            <div style={{ padding: '0 0 12px', borderBottom: '1px solid var(--card-dark-border)', marginBottom: 14, display: 'flex', alignItems: 'baseline', gap: 8 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-em)', margin: 0 }}>Tableau d&apos;amortissement</p>
              <p style={{ fontSize: 11, color: 'var(--text-muted-c)', margin: 0 }}>Capital remboursé vs restant dû sur {inputs.years} ans</p>
            </div>
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--card-dark-border)', display: 'flex', alignItems: 'baseline', gap: 8 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-em)', margin: 0 }}>Décomposition du coût</p>
              <p style={{ fontSize: 11, color: 'var(--text-muted-c)', margin: 0 }}>Total déboursé sur {inputs.years} ans</p>
            </div>
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} style={{ borderBottom: '1px solid var(--card-dark-border)' }}>
                    ...
                    <td style={{ padding: '10px 16px', textAlign: 'right', fontWeight: 600, fontVariantNumeric: 'tabular-nums', color: 'var(--text-em)' }}>{row.value}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', fontSize: 11, color: 'var(--text-muted-c)' }}>{row.pct ?? ''}</td>
                  </tr>
                ))}
                <tr style={{ background: 'rgba(255,255,255,0.03)' }}>
                  <td style={{ padding: '10px 16px', fontWeight: 600, color: 'var(--text-em)' }}>Total déboursé</td>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, padding: '14px 16px' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-em)', margin: '0 0 12px' }}>Répartition du total déboursé</p>
                ...
                {pieData.map(e => (
                  <div key={e.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <span style={{ fontSize: 10, color: 'var(--text-subtle)', marginLeft: 5 }}>{Math.round(e.value / (inputs.amount + r.totalCost) * 100)}%</span>
                    </div>
                  </div>
                ))}
              </div>
