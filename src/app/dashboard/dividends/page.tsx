...
export default function DividendsPage() {
  ...
  const PIE_COLORS = ['#34d399', '#60a5fa', '#f1c086', '#f87171', '#a78bfa', '#fb923c']
  ...
  return (
    <div style={{ padding: '20px 24px 48px' }}>
        ...
        {loading ? [1, 2, 3, 4].map(i => <SkeletonCard key={i} />) : (
          <>
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 12, padding: '12px 14px' }}>
              <p style={{ fontSize: 10, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 4px' }}>Dividendes / an</p>
            ...
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 12, padding: '12px 14px' }}>
              <p style={{ fontSize: 10, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 4px' }}>Dividendes / mois</p>
            ...
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 12, padding: '12px 14px' }}>
              <p style={{ fontSize: 10, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 4px' }}>Titres dividendes</p>
              <p style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-em)', fontVariantNumeric: 'tabular-nums', margin: 0, letterSpacing: '-0.5px' }}>
                {withDividends.length}<span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted-c)', marginLeft: 4 }}>/ {totalPositions}</span>
              </p>
            </div>
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 12, padding: '12px 14px' }}>
              <p style={{ fontSize: 10, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 4px' }}>Yield moyen pondéré</p>
      ...
      {!loading && totalPositions === 0 && (
        <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 16, padding: '48px 24px', textAlign: 'center' }}>
          <TrendingUp style={{ width: 40, height: 40, color: '#34d399', margin: '0 auto 12px' }} />
          <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-em)', marginBottom: 6 }}>Aucune position dans votre portefeuille</p>
          <p style={{ fontSize: 13, color: 'var(--text-muted-c)', maxWidth: 400, margin: '0 auto' }}>Ajoutez des actions ou ETF dans votre portefeuille pour estimer vos revenus passifs.</p>
        </div>
      )}

      {!loading && totalPositions > 0 && withDividends.length === 0 && (
        <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 16, padding: '48px 24px', textAlign: 'center' }}>
          <Coins style={{ width: 40, height: 40, color: 'var(--text-subtle)', margin: '0 auto 12px' }} />
          <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-em)', marginBottom: 6 }}>Aucun dividende détecté</p>
          <p style={{ fontSize: 13, color: 'var(--text-muted-c)', maxWidth: 460, margin: '0 auto' }}>Vos positions n'ont pas de rendement dividende détecté via Yahoo Finance. Cela peut concerner des ETF de capitalisation ou des données momentanément indisponibles.</p>
        </div>
      )}
      ...
      {hasData && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 290px', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, padding: '14px 16px' }}>
              <div style={{ marginBottom: 12 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-em)', margin: 0 }}>Top {chartData.length} — Dividendes annuels</p>
                <p style={{ fontSize: 11, color: 'var(--text-muted-c)', margin: 0 }}>Positions les plus rémunératrices</p>
              </div>
            ...
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 90px 110px 110px', borderBottom: '1px solid var(--card-dark-border)', padding: '10px 16px', background: 'var(--row-hover)' }}>
                {['Titre', 'Quantité', 'Cours', 'Rendement', 'Dividende/an', 'Dividende/mois'].map(col => (
                  <span key={col} style={{ fontSize: 10, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.07em', fontWeight: 600 }}>{col}</span>
                ))}
              </div>
              {data?.positions.map((p, idx) => {
                ...
                return (
                  <div key={p.id} style={{
                    ...
                    padding: '11px 16px', borderBottom: idx < (data?.positions.length ?? 0) - 1 ? '1px solid var(--section-border)' : 'none',
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--section-border)', fontSize: 11, color: 'var(--text-subtle)', lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* Donut répartition */}
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, padding: '14px 16px' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-em)', margin: '0 0 12px' }}>Répartition dividendes</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
                {pieData.map((d, i) => (
                  <div key={d.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-em)', fontVariantNumeric: 'tabular-nums' }}>{fmt(d.value)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.15)', borderRadius: 14, padding: '14px 16px' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-em)', margin: '0 0 12px' }}>Analyse revenus</p>
            ...
            {/* Tips */}
            <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, padding: '14px 16px' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-em)', margin: '0 0 10px' }}>Conseils rendement</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
