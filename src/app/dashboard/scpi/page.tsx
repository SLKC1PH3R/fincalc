      ...
      <div style={{ display: 'grid', gridTemplateColumns: '270px 1fr 290px', gap: 16, alignItems: 'start' }}>
        ...
        <div style={{ position: 'sticky', top: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {TMI_OPTIONS.map(t => (
                    <button key={t} onClick={() => set('tmi')(t)}
                      style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 6, cursor: 'pointer', border: `1px solid ${inputs.tmi === t ? COLOR : 'rgba(255,255,255,0.1)'}`, background: inputs.tmi === t ? `${COLOR}20` : 'transparent', color: inputs.tmi === t ? COLOR : 'var(--p-text-faint)', transition: 'all 0.15s' }}>
                      {t}%
                    </button>
                  ))}
                </div>
              ...
              <div style={{ background: `${COLOR}08`, border: `1px solid ${COLOR}20`, borderRadius: 10, padding: '10px 12px' }}>
                ...
                <p style={{ fontSize: 11, color: 'var(--p-text-dim)', lineHeight: 1.5 }}>Les revenus SCPI sont imposés comme revenus fonciers : TMI + 17.2% de prélèvements sociaux.</p>
          ...
          <div style={{ background: `${COLOR}0d`, border: `1px solid ${COLOR}25`, borderRadius: 12, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {[
              ...
              { label: 'Capital investi eff.', value: fmt(r.capitalInvested), color: 'var(--p-text-dim)' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            <div style={{ padding: '14px 18px', borderRadius: 12, background: `linear-gradient(135deg, ${COLOR}10, transparent)`, border: `1px solid ${COLOR}30`, position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ fontSize: 9, color: 'var(--p-text-faint)', textTransform: 'uppercase', letterSpacing: '0.10em', marginBottom: 4 }}>Revenu net / an</p>
              <p style={{ fontSize: 20, fontWeight: 800, color: COLOR, letterSpacing: '-0.5px', lineHeight: 1 }}>{fmt(r.netIncome)}</p>
              <p style={{ fontSize: 10, color: 'var(--p-text-faint)', marginTop: 4 }}>{fmt(Math.round(r.netIncome / 12))}/mois net</p>
            </div>
            {[
              { label: 'Rendement net', value: `${r.netYield.toFixed(2)}%`, sub: `brut ${inputs.distributionRate}% − fisc. ${r.taxRate.toFixed(1)}%`, color: 'var(--p-text)' },
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 12, padding: 14 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--p-text)', marginBottom: 10 }}>Évolution du patrimoine sur {inputs.duration} ans</p>
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 12, padding: '16px 20px' }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--p-text)', marginBottom: 12 }}>Détail de la rentabilité</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                ...
                <div key={k.label} style={{ background: k.highlight ? `${COLOR}08` : 'rgba(255,255,255,0.02)', border: `1px solid ${k.highlight ? COLOR + '25' : 'rgba(255,255,255,0.06)'}`, borderRadius: 10, padding: '12px 14px' }}>
                  ...
                  <p style={{ fontSize: 17, fontWeight: 800, color: k.highlight ? COLOR : 'var(--p-text)', letterSpacing: '-0.03em' }}>{k.value}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 12, padding: '14px 16px' }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--p-text)', marginBottom: 10 }}>Répartition sur {inputs.duration} ans</p>
          ...
          {/* SCPI référence */}
          <div style={{ background: 'var(--p-card)', border: '1px solid var(--p-line)', borderRadius: 12, padding: '14px 16px' }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--p-text)', marginBottom: 10 }}>SCPI populaires (indicatif 2024)</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                ...
                <div key={s.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--p-text)', margin: 0 }}>{s.name}</p>
                    <p style={{ fontSize: 10, color: 'var(--p-text-faint)', margin: 0 }}>{s.type}</p>
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 800, color: COLOR }}>{s.td}</span>
                </div>
              ))}
            </div>
            <p style={{ fontSize: 10, color: 'var(--p-text-faint)', marginTop: 8 }}>Taux de distribution 2024 — indicatif. Source : ASPIM.</p>
          </div>
