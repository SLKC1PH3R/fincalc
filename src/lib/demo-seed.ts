// src/lib/demo-seed.ts — Seed démo TypeScript (utilisé par l'API admin /reset)
...
export async function runDemoSeed(): Promise<{ simulations: number; envelopes: number; positions: number }> {
  // Upsert allowed emails
  const allowedEmails = ['admin@example.com', 'user1@example.com', DEMO_EMAIL]
  ...
  for (const sim of SIMS) {
    await prisma.simulation.create({
      data: { userId: demoUser.id, type: sim.type, name: sim.name, inputs: sim.inputs as object, results: sim.results as object },
    })
  }
  ...
  // Create positions
  for (const pos of POSITION_DEFS) {
    const envelopeId = envelopeMap[pos.envelope]
  ...
  return { simulations: SIMS.length, envelopes: ENVELOPE_DEFS.length, positions: POSITION_DEFS.length }
}
