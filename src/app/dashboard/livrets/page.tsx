        ...
        <div style={{ position: 'sticky', top: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {[0, 100, 200, 300].map(v => (
                    <button key={v} onClick={() => set('monthly')(v)}
                      style={{ flex: 1, padding: '3px 0', borderRadius: 6, fontSize: 10, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s',
                        ...
                        border: inputs.monthly === v ? `1px solid ${COLOR}35` : '1px solid var(--card-dark-border)',
                        color: inputs.monthly === v ? COLOR : 'var(--text-muted-c)' }}>
                      {v === 0 ? 'Aucun' : `${v}€`}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <Label style={{ fontSize: 11, color: 'var(--text-muted-c)' }}>Durée</Label>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11 }}>
                  {[5, 10, 15, 20, 30].map(y => (
                    <button key={y} onClick={() => set('duration')(y)}
                      style={{ background: inputs.duration === y ? `${COLOR}18` : 'transparent', border: inputs.duration === y ? `1px solid ${COLOR}35` : '1px solid transparent', borderRadius: 5, padding: '2px 6px', cursor: 'pointer', color: inputs.duration === y ? COLOR : 'var(--text-muted-c)', fontSize: 10, transition: 'all 0.15s' }}>
                      {y}a
                    </button>
                  ))}
                </div>
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 12, padding: '12px 14px' }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 8px' }}>Résumé</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                { label: 'Capital investi', value: fmt(totalInvested), color: 'var(--text-em)' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
            {[
              ...
              { label: 'ETF 7% net', value: fmt(r.etfNetFinal), sub: 'Flat Tax 30%', color: 'var(--text-primary)' },
              { label: 'Manque à gagner', value: fmt(r.opportunity), sub: 'vs ETF', color: 'var(--text-primary)' },
              { label: 'Intérêts livret', value: fmt(bestLivret.interest), sub: 'exonérés IR+PS', color: 'var(--text-primary)' },
            ].map((kpi, i) => (
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--card-dark-border)', display: 'flex', alignItems: 'baseline', gap: 8 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-em)', margin: 0 }}>Comparaison à {inputs.duration} ans</p>
              <p style={{ fontSize: 11, color: 'var(--text-muted-c)', margin: 0 }}>Tous les livrets réglementés + ETF</p>
            </div>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--card-dark-border)', background: 'var(--row-hover)' }}>
                  {['Produit', 'Taux net', 'Plafond', 'Valeur finale', 'Intérêts'].map((h, i) => (
                    <th key={h} style={{ padding: '8px 12px', fontSize: 9.5, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: i === 0 ? 'left' : 'right', fontWeight: 600 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {r.livrets.map(l => (
                  <tr key={l.name} style={{ borderBottom: '1px solid var(--card-dark-border)', background: l.name === bestLivret.name ? `${COLOR}06` : 'transparent' }}>
                    ...
                    <td style={{ padding: '10px 12px', fontSize: 12, color: 'var(--text-muted-c)', textAlign: 'right' }}>{fmt(l.cap)}</td>
                    <td style={{ padding: '10px 12px', fontSize: 14, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt(l.final)}</td>
                ...
                <tr style={{ background: 'rgba(129,140,248,0.06)', borderBottom: '1px solid var(--card-dark-border)' }}>
                  <td style={{ padding: '10px 12px', fontSize: 13, color: 'var(--text-em)', fontWeight: 600 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <td style={{ padding: '10px 12px', fontSize: 12, color: 'var(--text-subtle)', textAlign: 'right' }}>Illimité</td>
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, padding: '14px 16px' }}>
            <div style={{ paddingBottom: 12, borderBottom: '1px solid var(--card-dark-border)', marginBottom: 14, display: 'flex', alignItems: 'baseline', gap: 8 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-em)', margin: 0 }}>Évolution comparative</p>
              <p style={{ fontSize: 11, color: 'var(--text-muted-c)', margin: 0 }}>Livret A · LEP · ETF sur {inputs.duration} ans</p>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.18)', borderRadius: 14, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
              ...
              <div>
                <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-em)', margin: 0 }}>Analyse du rendement</p>
          ...
          <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, padding: '14px 16px' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-em)', margin: '0 0 12px' }}>Intérêts générés par produit</p>
