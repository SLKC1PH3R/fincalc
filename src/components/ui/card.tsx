...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, style, ...props }, ref) => (
    <div ref={ref} className={cn("rounded-lg text-card-foreground shadow-sm", className)}
      style={{ background: 'radial-gradient(ellipse at top left, rgba(241,192,134,0.09), transparent 70%)', border: '1px solid rgba(241,192,134,0.2)', ...style }}
      {...props} />
  )
)
...
const CardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("text-[0.75rem] text-muted-foreground", className)} {...props} />
  )
)
