      ...
      <div style={{ padding: '14px 24px 12px', borderBottom: '1px solid var(--section-border)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.3px' }}>Emprunts</h1>
            <span style={{ fontSize: 12, color: 'var(--text-subtle)', fontWeight: 400 }}>Crédits immobiliers simulés</span>
          </div>
      ...
      {!loading && sims.length === 0 && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center', padding: '48px 24px', background: 'var(--card-dark)', border: '2px dashed var(--card-dark-border)', borderRadius: 16, maxWidth: 420 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 6 }}>Aucun crédit simulé</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr 270px', gap: 16, alignItems: 'start' }}>
            ...
            <div style={{ position: 'sticky', top: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Simulations</span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = COLOR + '55'; (e.currentTarget as HTMLElement).style.background = 'var(--row-hover)' }}
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sim.name}</div>
                            <div style={{ fontSize: 10, color: 'var(--text-subtle)' }}>Crédit immobilier</div>
                          </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                          ...
                          {monthly > 0 && <span style={{ fontSize: 10, color: 'var(--text-subtle)' }}>{fmtCompact(monthly)}/mois</span>}
                          <span style={{ fontSize: 10, color: 'var(--text-subtle)' }}>{part.toFixed(0)}%</span>
                        </div>
                ...
                <Link href="/dashboard/mortgage" style={{ textDecoration: 'none' }}>
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, color: 'var(--text-subtle)' }}>Simuler un prêt</span>
                  </div>
                </Link>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                {[
                  ...
                  <div key={i} style={{ padding: '12px 16px', borderRadius: 12, background: k.highlight ? `linear-gradient(135deg, ${k.color}10, transparent)` : 'var(--card-dark)', border: `1px solid ${k.highlight ? k.color + '30' : 'var(--card-dark-border)'}`, position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 500, marginBottom: 4 }}>{k.label}</div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: k.highlight ? k.color : 'var(--text-primary)', letterSpacing: '-0.5px', fontVariantNumeric: 'tabular-nums', lineHeight: 1, marginBottom: 3 }}>{k.value}</div>
              ...
              <div style={{ background: 'var(--card-dark)', border: '1px solid var(--card-dark-border)', borderRadius: 14, overflow: 'hidden' }}>
                <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--card-dark-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>Comparatif des emprunts</span>
                  <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>Données simulées</span>
                </div>
                        ...
                        >
                          <td style={{ padding: '9px 14px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                              ...
                              <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-primary)' }}>{sim.name}</span>
                            </div>
                          </td>
                          ...
                          <td style={{ padding: '9px 14px', textAlign: 'right', fontSize: 12, color: 'var(--text-muted-c)' }}>{sim.inputs?.years != null ? `${sim.inputs.years} ans` : '—'}</td>
                          <td style={{ padding: '9px 14px', textAlign: 'right', fontSize: 12, color: 'var(--text-muted-c)', fontVariantNumeric: 'tabular-nums' }}>{sim.inputs?.rate != null ? `${Number(sim.inputs.rate).toFixed(2)}%` : '—'}</td>
                          <td style={{ padding: '9px 14px', textAlign: 'right', fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>{sim.results?.monthlyPayment != null ? fmtCompact(sim.results.monthlyPayment) : '—'}</td>
                  ...
                  {sims.length > 1 && (
                    <tfoot>
                      <tr style={{ borderTop: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)' }}>
                        <td colSpan={1} style={{ padding: '9px 14px', fontSize: 10, fontWeight: 700, color: 'var(--text-muted-c)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Total</td>
                        ...
                        <td colSpan={2} />
                        <td style={{ padding: '9px 14px', textAlign: 'right', fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', fontVariantNumeric: 'tabular-nums' }}>{fmtCompact(totalMonthly)}/m</td>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)' }}>Charge mensuelle</span>
                    </div>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                      <span style={{ fontSize: 13 }}>📈</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)' }}>Coût des intérêts</span>
                    </div>
                    <p style={{ fontSize: 12, color: 'var(--text-muted-c)', lineHeight: 1.65, margin: 0 }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                      <span style={{ fontSize: 13 }}>🎯</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)' }}>Remboursement anticipé</span>
                    </div>
                    <p style={{ fontSize: 12, color: 'var(--text-muted-c)', lineHeight: 1.65, margin: 0 }}>
